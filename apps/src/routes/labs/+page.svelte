...
<main class="max-w-4xl mx-auto md:p-8 p-4 mt-4">
	...
	<section class="mt-8">
		<h1 class="sr-only">Interactive Labs & Experiments</h1>

		<ul class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4" aria-label="Lab projects">
			{#each data.projects as project}
				<li
					class="border border-dark-200 dark:border-midnight-700 p-4 rounded-lg transition-all duration-300 hover:border-azure-500/50 dark:hover:border-azure-500/30 hover:shadow-md group relative overflow-hidden"
				>
					<div
					...
					<article class="relative z-0">
						<header>
							<h2
								class="text-lg font-semibold text-midnight-800 dark:text-dark-100 group-hover:text-azure-600 dark:group-hover:text-azure-400 transition-colors duration-200"
							>
								{project.name}
							</h2>
						</header>

						<p class="text-sm text-dark-600 dark:text-dark-300">{project.description}</p>

						<footer class="mt-2 text-xs text-dark-500 dark:text-dark-400">
							{#if project.author}
								<address class="not-italic">By {project.author}</address>
							{/if}
							<data value={project.version}>Version {project.version}</data>
						</footer>

						<div
							class="mt-2 inline-flex items-center text-azure-600 dark:text-azure-400 hover:underline transition-all duration-200 pointer-events-none"
						>
