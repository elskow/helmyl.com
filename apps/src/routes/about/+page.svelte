	...
	<div class="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-12 lg:gap-24">
		...
		>
			<div class="md:sticky md:top-24 space-y-8 md:space-y-12">
				...
				<div>
					<h3 class="text-xs text-neutral-400 mb-6 uppercase tracking-wide select-none">
						Connect
					</h3>
...
<style>
	...
	:global(.about-content li:has(strong)) {
		display: grid;
	...
	:global(.about-content li:has(strong):last-child) {
		border-bottom: none;
	}

	:global(.about-content li:has(strong)::before) {
		content: none;
	}

	:global(.about-content li strong) {
		color: #171717;
		font-weight: 500;
		font-size: 0.875rem;
	}
	...
	@media (max-width: 640px) {
		...
		:global(.about-content li:has(strong) strong) {
			color: #a3a3a3;
			text-transform: uppercase;
			font-size: 0.75rem;
			letter-spacing: 0.05em;
