<script lang="ts">
	...
	export let article = false, author,
		 ...
		 */
		breadcrumbs, datePublished, entity, lastUpdated, featuredImage, metadescription, siteLanguage, siteTitle,
		siteTitleAlt, siteUrl: string, title, url, facebookPage, githubPage, linkedinProfile, telegramUsername,
		tiktokUsername, twitterUsername,
		/**
	...
	const schemaOrgBreadcrumbList = {
		'@type': 'BreadcrumbList',
		'@id': `${url}#breadcrumb`,
		itemListElement: breadcrumbs.map((element: { name: string; slug: string }, index: number) => ({
			'@type': 'ListItem',
			position: index + 1,
			item: {
				'@type': 'WebPage',
